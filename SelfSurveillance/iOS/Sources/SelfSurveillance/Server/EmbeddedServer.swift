...
public final class EmbeddedServer {
    ...
    public func start() {
        ...
        // Wire up live-entry broadcast from ImmutableLogStore
    ...
    private func readRequest(_ conn: NWConnection, buffer: Data) {
        conn.receive(minimumIncompleteLength: 1, maximumLength: 32_768) { [weak self] data, _, _, error in
            guard let self = self else { return }
            guard let chunk = data, !chunk.isEmpty, error == nil else { return }
            var buf = buffer + chunk
            // Wait until we have the full HTTP header block (ends with \r\n\r\n)
            ...
            self.route(conn: conn, headers: headerBlock)
        }
    }

    // MARK: - Routing

    private func route(conn: NWConnection, headers raw: String) {
        let lines = raw.components(separatedBy: "\r\n")
        let parts = (lines.first ?? "").components(separatedBy: " ")
        guard parts.count >= 2 else { return }
        let path  = parts[1].components(separatedBy: "?")[0]
        let query = parts[1].contains("?") ? String(parts[1].dropFirst(path.count + 1)) : ""
        ...
        if hdr["upgrade"]?.lowercased() == "websocket", let key = hdr["sec-websocket-key"] {
            upgradeWebSocket(conn: conn, path: path, key: key)
            return
        }
        ...
        case "/api/logs":
            ...
            let json = (try? enc.encode(entries)) ?? Data("[]".utf8)
            send(conn, status: 200, contentType: "application/json", body: json)

        case "/api/logs/summary":
            let params   = parseQuery(query)
            let deviceID = params["deviceID"]
            let summary  = ImmutableLogStore.shared.querySummary(deviceID: deviceID)
            let json     = (try? JSONSerialization.data(withJSONObject: summary)) ?? Data("{}".utf8)
            send(conn, status: 200, contentType: "application/json", body: json)
    ...
    public func broadcastLiveEntry(_ entry: LogEntry) {
        ...
        let frame = wsFrame(data: json, opcode: 0x1) // text
        lock.lock()
    ...
    public func broadcastScreenFrame(_ jpeg: Data) {
        let frame = wsFrame(data: jpeg, opcode: 0x2) // binary
        lock.lock()
